<x-app-shell title="Dashboard Teller" header="Teller">
                    ...
                    <tbody id="document-table-body">
                        ...
                        @forelse($documents as $doc)
                            ...
                            <tr class="border-b hover:bg-gray-50 transition {{ $rowClass }}">
                                ...
                                {{-- Menampilkan Nama Lengkap Kategori --}}
        ...
        <div class="bg-white w-full max-w-2xl rounded-2xl border shadow-2xl max-h-[90vh] overflow-y-auto">
            <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    {{-- Kategori Teller Baru --}}
                    <div>
                        <label class="block text-sm font-medium mb-1">Kategori</label>
                        <select id="kategori_teller" name="category"
                            class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                            required>
                            <option value="" selected disabled>Pilih Kategori...</option>
                            <option value="TL-ST"
                                {{ old('form_selector') == 'uploadModal' && old('category') == 'TL-ST' ? 'selected' : '' }}>
                                Transaksi setoran dan penarikan (TL-ST)</option>
                            <option value="TL-TP"
                                {{ old('form_selector') == 'uploadModal' && old('category') == 'TL-TP' ? 'selected' : '' }}>
                                Transaksi transfer dan pembayaran (TL-TP)</option>
                            <option value="TL-GK"
                                {{ old('form_selector') == 'uploadModal' && old('category') == 'TL-GK' ? 'selected' : '' }}>
                                Transaksi Giro, Kliring, Valuta (TL-GK)</option>
                            <option value="TL-LA"
                                {{ old('form_selector') == 'uploadModal' && old('category') == 'TL-LA' ? 'selected' : '' }}>
                                Laporan dan Administrasi teller (TL-LA)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input id="nomor_dokumen_teller" type="text" name="document_number"
                            value="{{ old('form_selector') == 'uploadModal' ? old('document_number') : '' }}"
                            class="w-full rounded-lg border-gray-300 bg-gray-100 cursor-not-allowed focus:ring-blue-500 focus:border-blue-500"
                            placeholder="-" readonly required>
                        <p class="text-xs text-gray-500 mt-1">Digenerate otomatis by sistem</p>
                    </div>
    ...
    {{-- SCRIPTS --}}
    <script>
        const kategoriTeller = document.getElementById('kategori_teller');
        const noDocTeller = document.getElementById('nomor_dokumen_teller');

        if (kategoriTeller) {
            kategoriTeller.addEventListener('change', function() {
                const prefix = this.value;
                if (!prefix) return;

                noDocTeller.value = 'Loading...';

                fetch(`{{ route('documents.generate-number') }}?prefix=${prefix}`)
                    .then(res => res.json())
                    .then(data => {
                        noDocTeller.value = data.number;
                    })
                    .catch(err => {
                        console.error(err);
                        noDocTeller.value = 'Error';
                    });
            });
        }
        // Fungsi Filter
            ...
            setInterval(() => {
                ...
                fetch(window.location.href, {
                    ...
                    .then(html => {
                        ...
                        const newStats = doc.getElementById('teller-stats');
                        const currentStats = document.getElementById('teller-stats');
                        if (newStats && currentStats) {
                            currentStats.innerHTML = newStats.innerHTML;
                        }

                        const newTable = doc.getElementById('teller-table');
                        const currentTable = document.getElementById('teller-table');

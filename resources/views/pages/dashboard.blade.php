    ...
    <div class="bg-white rounded-xl border p-6 shadow-sm mb-6">
            ...
            <div class="flex gap-4 text-xs font-medium">
                <div class="flex items-center gap-1.5">
                    <span class="w-3 h-3 rounded-full bg-green-500"></span> Aktif (< 4 Tahun) </div>
                        <div class="flex items-center gap-1.5">
                            <span class="w-3 h-3 rounded-full bg-yellow-500"></span> Masa Retensi (4-5 Tahun)
                        </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border mt-6 shadow-sm overflow-hidden">
            ...
            <div class="p-6 border-b flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                {{-- Tab Filter Lifecycle (Aktif vs Masa Retensi) --}}
                <div class="flex bg-gray-100 p-1 rounded-lg">
                    ...
                    <button onclick="filterLifecycle('retention')" id="tab-retention"
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody id="document-table-body">
                        @forelse($documents as $doc)
                            @php
                                $lifecycleStatus = $doc->is_active ? 'active' : 'retention';
                                // Baris agak kuning jika dalam masa retensi (Warning)
                                $rowClass = !$doc->is_active
                                    ? 'bg-yellow-50/50 text-gray-700'
                                    : 'bg-white text-gray-900';
                            @endphp

                            <tr class="border-b transition hover:bg-gray-50 document-row {{ $rowClass }}"
                                data-approval="{{ $doc->status }}"
                                ...
                                <td class="py-3 px-4">
                                    @if (!$doc->is_active)
                                        <span
                                            ...
                                            title="Dokumen segera dihapus otomatis">
                                            MASA RETENSI
                                        </span>
                                    @else
                                        <span
                            ...
                            {{-- MODAL DETAIL & EDIT (Include Component yang sama) --}}
                            <div id="detailModal-{{ $doc->id }}"
                                ...
                                <div
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2 mb-1">
                                                ...
                                                @elseif(!$doc->is_active)
                                                    <span
                                                        class="px-2 py-0.5 rounded text-[10px] font-bold bg-gray-200 text-gray-600">ARSIP</span>
                                                @endif
                                            </div>
                                    ...
                                    {{-- View Section (Sama seperti sebelumnya, hanya styling minor) --}}
                                    <div id="viewSection{{ $doc->id }}" class="p-6">
                                        @if ($doc->status == 'rejected')
                                            <div
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm mt-1">Alasan: {{ $doc->rejection_reason }}</p>
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div
                                                ...
                                                @if ($doc->file_path)
                                                    <div class="text-center">
                                                        {{-- Gunakan Icon PDF jika bukan gambar --}}
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div>
                                                    <label class="text-xs font-bold text-gray-500 uppercase">Diunggah
                                                        Oleh</label>
                                        ...
                                        <div class="mt-8 flex justify-end gap-3 pt-5 border-t">
                                            ...
                                            {{-- Tombol Hapus Permanent (Jika Status Sampah/Trashed) --}}
                                    ...
                                    {{-- Edit Section (Include Form Edit Anda yang sudah ada, disembunyikan scr default) --}}
                                    <div id="editSection{{ $doc->id }}" class="hidden">
                                        <form action="{{ route('documents.update', $doc->id) }}" method="POST"
                                            enctype="multipart/form-data">
                                            @csrf @method('PUT')
                                            {{-- Isi Form Edit Sama Seperti Code Anda Sebelumnya --}}
                                            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {{-- (Paste ulang isi form edit Anda disini agar code tidak terlalu panjang) --}}
                                                {{-- SHORTCUT: Input field form edit sama persis seperti kode anda sebelumnya --}}
                                                ...
                                                <div><label class="block text-sm font-medium mb-1">Nomor
                                                        Dokumen</label><input type="text" name="document_number"
                                                ...
                                                <div><label class="block text-sm font-medium mb-1">Rak</label><select
                                                ...
                                                <div><label class="block text-sm font-medium mb-1">Box</label><select
        ...
        <script>
            ...
            // Filter 1: Berdasarkan Kartu Atas (Approved/Rejected)
            ...
            // Filter 2: Berdasarkan Tab Lifecycle (Aktif/Arsip/Sampah)
            ...
            function applyFilters() {
                ...
                rows.forEach(row => {
                    ...
                    const lifecycle = row.dataset.lifecycle; // active, archive, trash
                    ...
                    else if (currentApprovalFilter === 'today') matchApproval = (today === 'yes');

                    // Cek Filter Lifecycle
                ...
                document.getElementById('filtered-count').textContent = `(${visibleCount} data)`;
            }

<x-app-shell title="Dashboard Utama" header="Dashboard">
    ...
    <div class="bg-white rounded-xl border p-6 shadow-sm">
        ...
        <div class="text-sm text-gray-600 mt-1">
            Monitoring seluruh dokumen masuk dari Teller dan Customer Service.
        </div>
    </div>
                ...
                <tbody id="document-table-body">
                    @forelse($documents as $doc)
                        <tr class="border-b hover:bg-gray-50 document-row transition" data-status="{{ $doc->status }}"
                            ...
                            <td class="py-3 px-4">{{ \Carbon\Carbon::parse($doc->document_date)->format('d M Y') }}</td>
                                ...
                                <div
                                    ...
                                    <div>
                                        <div class="text-xs font-bold text-gray-500 uppercase tracking-wide">
                                            Detail Dokumen {{ strtoupper($doc->source) }}
                                        </div>
                                ...
                                <div id="viewSection{{ $doc->id }}" class="p-6">
                                    ...
                                    @if ($doc->status == 'rejected')
                                        <div
                                            ...
                                            <div>
                                                ...
                                                <p class="mt-2 text-xs text-red-600 font-semibold">Silakan perbaiki data
                                                    atau upload ulang dokumen.</p>
                                            </div>
                                        </div>
                                    @elseif($doc->status == 'approved')
                                        <div
                                            ...
                                            <div>
                                                <h4 class="font-bold">Status: Disetujui</h4>
                                                <p class="text-sm mt-1">Dokumen telah diverifikasi. Data terkunci.</p>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    <div class="mt-8 flex justify-end gap-3 pt-5 border-t">
                                        ...
                                        {{-- LOGIKA TOMBOL: HANYA TAMPIL JIKA BUKAN SUPERVISOR --}}
                                        @if (auth()->user()->role !== 'supervisor')
                                            {{-- 1. BUTTON HAPUS: HANYA MUNCUL JIKA STATUS REJECTED --}}
                                            @if ($doc->status == 'rejected')
                                            ...
                                            {{-- 2. BUTTON EDIT: MUNCUL JIKA PENDING ATAU REJECTED --}}
                                    ...
                                    <form action="{{ route('documents.update', $doc->id) }}" method="POST"
                                        ...
                                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div class="md:col-span-2 text-center border-b pb-4 mb-2">
                                                <label class="block text-sm font-bold text-gray-700 mb-2">Ganti File
                                                    (Opsional)</label>
                                                <input type="file" name="file_path"
                                                    class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                                            </div>

                                            {{-- Form Input Berdasarkan Source --}}
                                            @if ($doc->source == 'cs')
                                                <div>
                                            ...
                                            <div class="md:col-span-2 border-t pt-2 mt-2 font-semibold text-gray-700">
                                                Lokasi Fisik</div>
                                            <div>
    ...
    <script>
        ...
        function openDetail(id) {
            ...
            if (modal) {
                ...
                toggleEditMode(id, false); // Pastikan selalu reset ke view mode saat buka
            }
        }
        ...
        function filterDocuments(filter) {
            ...
            rows.forEach(row => {
                ...
                switch (filter) {
                    case 'all':
                        shouldShow = true;
                        tableTitle.textContent = 'Semua Dokumen';
                        break;
                    case 'approved':
                    ...
                    case 'rejected':
                        ...
                        tableTitle.textContent = 'Dokumen Rejected';
                        break;
                    case 'today':
                        shouldShow = row.dataset.today === 'yes';
                        tableTitle.textContent = 'Masuk Hari Ini';
                        break;
                }
            ...
            filteredCount.textContent = filter !== 'all' ? `(${visibleCount} dokumen)` : '';
